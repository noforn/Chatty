...
*   Google Generative AI (Gemini): The core AI model powering the assistant. (Specifically, `gemini-2.0-flash-live-001` and `gemini-2.0-flash-exp` as per the code).
...
*   **`light_agent/`**: This directory contains the core logic for the AI assistant.
    *   **`agent.py`**: Defines the "Chatty" agent, including its persona, instructions, and the tools it can use (light control, calendar management, file operations, memory, and web search). It also initializes the Gemini client and configures the agent's capabilities.
